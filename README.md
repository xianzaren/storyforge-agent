...
- 实体素材包：按连续事件自动组合基础片段，并导出包级预览 MP4、独立 clips、SRT、联系表和 JSON；跨时间相似内容只建立关联组，不错误拼接。
...
对于风景、环境声或不需要文字覆盖的视频，可把字幕来源设为 `none`（Web 中为“不添加字幕”），并选择 `source_original`。此时系统保留原音轨，不生成或烧录字幕，也不会强行添加 AI 旁白。离线主题模板同样不再包含固定的 StoryForge/工作流宣传语。
...
当前 37 项测试覆盖自动切片、短镜头合并、持续音频/文本情绪变化切点、瞬时噪声过滤、音频候选与画面切点融合、连续事件素材包、跨时间相似组、人工保留项和标签重建、包级 MP4/SRT/联系表/JSON 导出、相似镜头聚类、语音时间轴对齐、无语音不虚构内容，以及原有自动成片回归能力。
...
│  ├─ packager.py              # 连续事件素材包、相似组与实体文件导出
